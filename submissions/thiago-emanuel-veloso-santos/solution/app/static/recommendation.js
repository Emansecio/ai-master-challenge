...
const date = (s) => /^\d{4}-\d{2}-\d{2}$/.test(s) ? s.split('-').reverse().join('/') : 'Data não informada';
const missingLabels = { need: 'Necessidade', budget: 'Orçamento', authority: 'Participantes da decisão', intent: 'Intenção de compra', last_contact: 'Último contato', next_contact_date: 'Próximo contato' };
const identify = { title: 'Confirme a empresa', question: 'A qual empresa esta oportunidade pertence? Confira no CRM de origem antes de vincular.', record: 'Empresa confirmada e responsável pelo atendimento. Não escolha uma conta apenas para completar o campo.' };
const status = { title: 'Confira se a negociação segue ativa', question: 'Quando ocorreu o último contato e o que o cliente respondeu?', record: 'Data e resultado do contato, situação atual e eventual motivo de pausa ou encerramento, no CRM de origem.' };
const next = { title: 'Combine um próximo passo verificável', question: 'Qual ação foi combinada com o cliente, quem é responsável e para quando?', record: 'Ação, responsável e data confirmados no CRM de origem. Não presuma um compromisso ainda não acordado.' };
export function explainRecommendation(r) {
    const s = r.state, action = r.decision.next_action;
    const facts = [{ label: 'Empresa', value: s.account_identified ? r.account || 'Identificação a conferir' : 'Não vinculada' }, { label: 'Estágio registrado', value: s.stage === 'Engaging' ? 'Em negociação' : s.stage === 'Prospecting' ? 'Prospecção' : 'A revisar' }];
    const missing = (s.unavailable || []).map(key => missingLabels[key] || key);
    const result = { rule: 'Revisão dos dados', summary: 'Confira os dados antes de usar uma orientação comercial.', facts, steps: [{ title: 'Revise a origem dos dados', question: 'Qual informação está ausente ou contraditória? Compare estágio, empresa e datas com o registro original.', record: 'Correção confirmada na fonte. Reavalie a orientação somente depois da atualização.' }], missing, limit: 'Os dados não permitem estimar a chance de fechamento.', secondary: null, duration: null };
    // Invalid states have no duration graphic: numerical precision must not imply valid evidence.
    if (action === 'human_review' || !['identify_account', 'qualify_prospect', 'review_old_negotiation', 'continue_negotiation_review'].includes(action))
        return result;
    ...
    switch (action) {
        case 'identify_account':
            ...
            if (result.duration && result.duration.age > result.duration.reference)
                result.secondary = 'Há duas verificações neste caso: identificar a empresa e confirmar se a negociação continua ativa. A duração acima da referência continua relevante, mesmo com cadastro pendente.';
            break;
...
export function reasonPreview(r) {
    switch (r.decision.next_action) {
        case 'identify_account': {
            const d = explainRecommendation(r).duration;
            return d && d.age > d.reference ? 'Sem empresa; duração acima da referência' : 'Falta identificar a empresa';
        }
        case 'qualify_prospect': return 'Empresa identificada; prospecção';
        case 'review_old_negotiation': {
            const d = explainRecommendation(r).duration;
            return d ? d.comparison : 'Conferir duração e referência';
        }
        case 'continue_negotiation_review': return 'Confirmar o próximo compromisso';
        default: return 'Dados exigem revisão humana';
    }
}
export function conferenceSummary(r) {
