import { explainRecommendation, reasonPreview, conferenceSummary } from './recommendation.js';
const el = (id) => document.getElementById(id);
const esc = (s) => String(s ?? '').replace(/[&<>"']/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]));
const number = (n) => n.toLocaleString('pt-BR');
const currency = (n) => n.toLocaleString('pt-BR', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 });
let meta;
let page = 1;
let action = '';
let current;
...
function selectionQuery() {
    const query = new URLSearchParams({ page: String(page), action, search: el('search').value });
    for (const field of filters)
        query.set(field, el(field).value);
    return query;
}
function writeLocation(mode, query = selectionQuery()) {
...
function restoreLocation() {
    ...
    el('search').value = query.get('search') || '';
    for (const field of filters) {
        const value = query.get(field) || '';
        el(field).value = meta.filters[field].includes(value) ? value : '';
    }
}
function navigate(scrollToResults = false) { clearTimeout(debounce); writeLocation('push'); void load(scrollToResults); }
function rememberDraft(r) {
    const account = el('account-select').value;
    if (account === r.account)
        drafts.delete(r.id);
    else
        drafts.set(r.id, { version: r.version, account });
    el('save-account').disabled = account === r.account || saving.has(r.id);
...
function stamp(value) { return new Date(value).toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' }) + ' (Brasília)'; }
async function showAudit(r) {
    const panel = el('audit-content');
    if (!panel.textContent)
        panel.textContent = 'Carregando histórico…';
    panel.setAttribute('aria-busy', 'true');
    const epoch = sessionEpoch;
    try {
        const a = await api('/api/opportunities/' + encodeURIComponent(r.id) + '/audit');
        if (epoch !== sessionEpoch || !panel.isConnected || current?.id !== r.id)
            return;
        const origins = { ensaio_validado: 'Ensaio real importado', consulta_ao_vivo: 'Consulta ao vivo', test_fixture: 'Resposta controlada de teste' };
        const outcomes = { succeeded: 'Concluída', stale: 'Resultado desatualizado', pending: 'Falha transitória; nova tentativa agendada', failed: 'Falha; revisão técnica necessária', interrupted: 'Interrupção detectada: reserva de processamento vencida' };
        const expanded = Array.from(panel.querySelectorAll('details[id][open]')).map(d => d.id);
        ...
        if (panel.isConnected && epoch === sessionEpoch)
...
function renderMessage() {
    ...
    const inline = !!copy && notice?.placement === 'copy' && notice.opportunityId === current?.id && el('conference-summary').open && (!compactDetail.matches || detailDialog.open);
    el('message').textContent = inline ? '' : notice?.opportunityId ? `${notice.opportunityId}: ${text}` : text;
    el('message').hidden = !text || inline;
    const local = document.getElementById('detail-message');
    if (local) {
        const applies = !notice?.opportunityId || notice.opportunityId === current?.id;
        local.textContent = applies && !inline ? text : '';
        local.hidden = !applies || !text || inline;
    }
    if (copy) {
        copy.textContent = inline ? text : '';
        copy.hidden = !inline || !text;
    }
}
function message(text, opportunityId, placement) { notice = text ? { text, opportunityId, placement } : undefined; renderMessage(); }
...
summaryPanel.open = !compactSummary.matches;
...
const detailDialog = el('detail-dialog');
let returnToRow;
...
function restoreDetail() {
    el('detail-pane').append(el('detail'));
    document.body.classList.remove('detail-open');
    if (!el('shell').hidden && returnToRow) {
        const button = Array.from(el('rows').querySelectorAll('button[data-id]')).find(b => b.dataset.id === returnToRow);
        (button || el('search')).focus({ preventScroll: true });
    }
...
detailDialog.addEventListener('keydown', event => {
    if (event.key !== 'Tab' || !detailDialog.open)
        return;
    const targets = Array.from(detailDialog.querySelectorAll('button:not(:disabled),a[href],input:not(:disabled),select:not(:disabled),textarea:not(:disabled),summary,[tabindex]:not([tabindex="-1"])')).filter(node => node.getClientRects().length > 0);
...
compactDetail.addEventListener('change', () => { if (!compactDetail.matches && detailDialog.open)
    detailDialog.close(); });
function openDetail(r) {
    detail(r);
    if (compactDetail.matches) {
        returnToRow = r.id;
        el('detail-mobile-host').append(el('detail'));
...
function resetSession(reason = '') {
    ...
    page = 1;
    action = '';
    for (const id of ['metric-total', 'metric-review', 'metric-discovery', 'metric-missing'])
        el(id).textContent = '—';
    el('summary-count').textContent = '';
    el('data-quality').hidden = true;
    ...
    el('login-error').textContent = reason;
    el('search').value = '';
...
async function api(path, options = {}) {
    ...
    try {
        ...
        if (e.name === 'AbortError')
            throw e;
        throw new Error('Não foi possível conectar. Confira se o servidor está disponível e tente atualizar.');
    }
    if (epoch !== sessionEpoch)
        throw new DOMException('Sessão alterada', 'AbortError');
    let value;
    try {
    ...
    if (response.status === 401)
        resetSession(value.error || 'Entre novamente.');
    if (!response.ok)
        throw new ApiError(value.error || 'Não foi possível concluir.', response.status);
    return value;
}
const statusLabel = (r) => ({ classified: 'Jev · coerente com a política', pending: 'Na fila', running: 'Classificando…', failed: 'Falha · regra local', stale: 'Desatualizado · regra local', unclassified: 'Política local' }[r.status] || 'Política local');
async function start() { el('retry-start').hidden = true; try {
    meta = await api('/api/meta');
    el('login').hidden = true;
    el('shell').hidden = false;
    el('user-name').textContent = meta.user.id;
    el('avatar').textContent = meta.user.id[0].toUpperCase();
    el('user-role').textContent = ({ admin: 'Administrador', manager: 'Gestor', seller: 'Vendedor' }[meta.user.role] || meta.user.role);
    for (const field of filters) {
        el(field).innerHTML = `<option value="">${field === 'region' ? 'Todas' : 'Todos'}</option>` + meta.filters[field].map(v => `<option>${esc(v)}</option>`).join('');
    }
    restoreLocation();
    await load();
}
    ...
    if (!el('shell').hidden)
        ...
        el('retry-start').hidden = e instanceof ApiError && e.status === 401;
    }
} }
async function load(scrollToResults = false) {
    ...
    el('results-region').setAttribute('aria-busy', 'true');
    el('load-status').textContent = 'Atualizando carteira…';
    el('prev').disabled = true;
    el('next').disabled = true;
    try {
        const value = await api('/api/opportunities?' + query, { signal: listRequest.signal });
        if (seq !== sequence)
            return;
        const focusedQueue = document.activeElement?.dataset.action;
        data = value;
        page = data.page;
        query.set('page', String(page));
        writeLocation('replace', query);
        render();
        if (focusedQueue !== undefined) {
            for (const button of el('queues').querySelectorAll('button'))
                if (button.dataset.action === focusedQueue)
                    button.focus();
        }
        if (current) {
            const fresh = data.rows.find(r => r.id === current.id);
            if (fresh)
                detail(fresh);
            else {
                current = undefined;
                el('detail').innerHTML = detailPlaceholder;
                if (detailDialog.open)
                    detailDialog.close();
                clearTimeout(poll);
            }
        ...
        if (scrollToResults) {
            el('portfolio-title').focus({ preventScroll: true });
            el('results-region').scrollIntoView({ block: 'start' });
        }
    }
    catch (e) {
        if (seq === sequence && e.name !== 'AbortError') {
            if (data)
                page = data.page;
            message(e.message);
            el('load-status').textContent = 'Falha ao atualizar. Os dados visíveis são da última consulta concluída.';
        }
    }
    finally {
        if (seq === sequence) {
            el('results-region').setAttribute('aria-busy', 'false');
            if (data) {
...
function render() {
    if (!data)
        return;
    const activeFilters = filters.filter(f => el(f).value).length;
    el('toggle-filters').textContent = activeFilters ? `Equipe (${activeFilters} ${activeFilters === 1 ? 'filtro' : 'filtros'})` : 'Filtrar equipe';
    const missing = data.counts.identify_account || 0;
    el('data-quality').hidden = missing === 0;
    el('data-quality-text').textContent = `${number(missing)} sem empresa vinculada (${(data.scope_total ? 100 * missing / data.scope_total : 0).toLocaleString('pt-BR', { maximumFractionDigits: 1 })}% da seleção). Perfil incompleto não indica menor potencial comercial.`;
    el('summary-count').textContent = number(data.scope_total) + ' oportunidades';
    el('metric-total').textContent = number(data.scope_total);
    el('metric-review').textContent = number(data.counts.review_old_negotiation || 0);
    el('metric-discovery').textContent = number(data.counts.qualify_prospect || 0);
    el('metric-missing').textContent = number(data.counts.identify_account || 0);
    ...
    el('queues').innerHTML = queues.map(([key, label, count]) => `<button class="queue ${key === action ? 'active' : ''}" data-action="${key}" aria-pressed="${key === action}">${label} <span>${number(Number(count))}</span></button>`).join('');
    el('rows').innerHTML = data.rows.map(r => `<tr role="row" data-id="${esc(r.id)}" class="${current?.id === r.id ? 'selected' : ''}"><td role="cell"><button class="open-deal" aria-controls="detail" aria-pressed="${current?.id === r.id}" data-id="${esc(r.id)}">${esc(r.account || 'Empresa não vinculada')} <span class="deal-product">${esc(r.state.product)}</span> <span class="deal-key">${esc(r.id)}</span></button></td><td role="cell"><span class="badge priority-${r.priority}">${esc(r.action_label)}</span><span class="decision-reason">${esc(reasonPreview(r))}</span><span class="subtext">${esc(r.sales_agent)}</span></td><td role="cell">${currency(r.state.catalog_price)}<span class="subtext">preço de catálogo</span></td><td role="cell"><span class="source ${r.classification ? 'jev' : ''}">${statusLabel(r)}</span></td></tr>`).join('');
    el('empty').hidden = data.total > 0;
    el('page-label').textContent = `${number(data.total)} ${data.total === 1 ? 'oportunidade' : 'oportunidades'} · página ${data.page} de ${data.pages}`;
    el('prev').disabled = data.page === 1;
    el('next').disabled = data.page === data.pages;
}
function detail(r) {
    const same = current?.id === r.id;
    const auditContent = same ? el('audit-content')?.innerHTML || '' : '';
    const expanded = same ? Array.from(el('detail').querySelectorAll('details[id][open]')).map(d => d.id) : [];
    const scrollTop = same ? el('detail').scrollTop : 0;
    const focusId = same && el('detail').contains(document.activeElement) ? document.activeElement.id : '';
    current = r;
    clearTimeout(poll);
    const draft = drafts.get(r.id);
    if (draft && draft.version !== r.version) {
        drafts.delete(r.id);
        message('A oportunidade mudou de versão. A seleção de conta não salva foi descartada; confira os dados atuais.', r.id);
    }
    for (const row of document.querySelectorAll('tr[data-id]')) {
        row.classList.toggle('selected', row.dataset.id === r.id);
        row.querySelector('button')?.setAttribute('aria-pressed', String(row.dataset.id === r.id));
    }
    const busy = ['pending', 'running'].includes(r.status);
    const classified = !!r.classification;
    const confidence = classified ? Object.entries(r.classification.answers).map(([q, a]) => `${q === 'qualification' ? 'Qualificação' : 'Próxima ação'}: ${Math.round(a.confidence * 100)}%`).join(' · ') : '';
    ...
    el('copy-summary').addEventListener('click', async () => {
        const epoch = sessionEpoch, button = el('copy-summary'), text = conferenceSummary(r);
        button.disabled = true;
        try {
            await navigator.clipboard.writeText(text);
            if (epoch === sessionEpoch)
                message(`Resumo da versão ${r.version} copiado. Atendimento não registrado.`, r.id, 'copy');
        }
        catch {
            if (epoch !== sessionEpoch)
                return;
            ...
            if (current?.id === r.id) {
                const field = el('conference-text');
                field.focus();
                field.select();
            }
        }
            ...
            if (epoch === sessionEpoch && current?.id === r.id) {
                const status = document.getElementById('copy-status');
                if (status && !status.hidden)
                    el('copy-summary').scrollIntoView({ block: 'nearest' });
            }
        }
    });
    const accountSelect = el('account-select');
    const restored = drafts.get(r.id);
    if (restored)
        accountSelect.value = restored.account;
    rememberDraft(r);
    accountSelect.addEventListener('change', () => rememberDraft(r));
    el('audit').addEventListener('toggle', () => { if (el('audit').open)
        void showAudit(r); });
    for (const id of expanded) {
    ...
    el('classify').addEventListener('click', async () => { const epoch = sessionEpoch; const button = el('classify'); button.disabled = true; message(''); try {
        await api('/api/classify', { method: 'POST', body: JSON.stringify({ id: r.id, version: r.version }) });
        const fresh = await api('/api/opportunities/' + encodeURIComponent(r.id));
        if (current?.id === r.id)
            detail(fresh);
        await load();
    }
    catch (e) {
        if (epoch === sessionEpoch && e.name !== 'AbortError')
            message(e.message, r.id);
        button.disabled = false;
    } });
    el('account-form').addEventListener('submit', async (event) => {
        ...
        if (account === r.account)
            return;
        saving.add(r.id);
        el('save-account').disabled = true;
        accountSelect.disabled = true;
        const epoch = sessionEpoch;
        try {
            const fresh = await api('/api/opportunities/' + encodeURIComponent(r.id), { method: 'PATCH', body: JSON.stringify({ version: r.version, account }) });
            if (epoch !== sessionEpoch)
                return;
            drafts.delete(r.id);
            message('Conta salva. A nova versão está registrada no histórico.', r.id);
            saving.delete(r.id);
            if (current?.id === r.id)
                detail(fresh);
            await load();
            ...
            saving.delete(r.id);
            if (epoch === sessionEpoch && current?.id === r.id) {
                el('account-select').disabled = false;
    ...
    if (busy) {
        const epoch = sessionEpoch;
        poll = window.setTimeout(async () => { try {
            const fresh = await api('/api/opportunities/' + encodeURIComponent(r.id));
            if (current?.id === r.id) {
                detail(fresh);
...
for (const link of document.querySelectorAll('a[href="#method"]'))
    link.addEventListener('click', event => { event.preventDefault(); el('method').open = true; el('method').scrollIntoView({ block: 'start' }); el('method').querySelector('summary')?.focus({ preventScroll: true }); });
el('retry-start').addEventListener('click', () => void start());
el('login-form').addEventListener('submit', async (e) => { e.preventDefault(); const button = e.currentTarget.querySelector('button'); button.disabled = true; el('login-error').textContent = ''; try {
    await api('/api/login', { method: 'POST', body: JSON.stringify({ code: el('access-code').value }) });
...
el('logout').addEventListener('click', async () => { if (drafts.size && !window.confirm('Há alterações de conta não salvas. Sair e descartá-las?'))
    return; try {
...
el('queues').addEventListener('click', e => { const button = e.target.closest('button[data-action]'); if (button) {
    action = button.dataset.action || '';
    page = 1;
    navigate();
} });
el('rows').addEventListener('click', e => { const button = e.target.closest('button[data-id]'); const row = data?.rows.find(r => r.id === button?.dataset.id); if (row)
    openDetail(row); });
for (const field of filters)
    el(field).addEventListener('change', () => { page = 1; navigate(); });
let debounce;
el('search').addEventListener('input', () => { clearTimeout(debounce); debounce = window.setTimeout(() => { page = 1; navigate(); }, 250); });
function clearFilters() { for (const field of filters)
    el(field).value = ''; el('search').value = ''; action = ''; page = 1; message(''); navigate(); }
el('clear').addEventListener('click', clearFilters);
el('clear-empty').addEventListener('click', () => { clearFilters(); el('search').focus(); });
el('refresh').addEventListener('click', () => { message(''); void load(); });
el('prev').addEventListener('click', () => { page--; navigate(true); });
el('next').addEventListener('click', () => { page++; navigate(true); });
window.addEventListener('popstate', () => { clearTimeout(debounce); if (el('shell').hidden)
    return; restoreLocation(); message(''); void load(); });
window.addEventListener('beforeunload', event => { if (drafts.size) {
    event.preventDefault();
    event.returnValue = '';
} });
void start();
